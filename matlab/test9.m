...
pI=[       0.0010   61.3451    0.7010  138.6009    3.8127    0.8693    7.8032    1.9592         0 ]';
...
%training data
data=experiments{34}.data; data(1:2000,:)=[];    % data=[data; experiments{5}.data];

%%%%%%%%%%%%%%%%%%%%%%%%%%%

%data to compare simulation
testdata=data;%experiments{5}.data;
dt=1e-4;
...
for i=1:nI
    Aeq=zeros(9,9); beq=zeros(9,1);
    options=gaoptimset('TimeLimit',Tl,'InitialPopulation',pI','UseParallel','always', ...
                       'PopulationSize',500,'Generations',4000,'StallGenLimit',1000,'PlotFcns', ...
                       {@gaplotbestf,@gaplotstopping,@gaplotbestindiv});

    if (mod(i,2)==0)
        Aeq(1:3,1:3)=eye(3); beq(1:3)=pI(1:3);
    else
        Aeq(4:9,4:9)=eye(6); beq(4:9)=pI(4:9);    
    end    

    [p,o,exitflag] = ga(obj,9,A,b,Aeq,beq,lb,ub,[],options);
